...
class MainForm(QMainWindow):
    def __init__(self):
        ...
        self.db.setDatabaseName('resources/db/products.db')
    ...
    def addToDb(self):
        char = self.ui.Name_lineEdit.text()
        if char == "" or char == " ":
            QMessageBox.question(self,'Message', "Veuillez entrer la désignation du produit !", QMessageBox.Ok)
            self.show()
        else :
    ...
    def delrow(self):
        if self.ui.tableWidget.currentIndex().row() > -1:
            ...
            QMessageBox.question(self,'Message', "Veuillez sélectionner une ligne que vous souhaitez supprimer !", QMessageBox.Ok)
            self.show()
    ...
    def updaterow(self):
        if self.ui.tableWidget.currentIndex().row() > -1:
            ...
            if(record.value("Quantity") == 0):
                ...
                msg = "Le produit '{}' n'est plus en stock"
            ...
            QMessageBox.question(self,'Message', "Veuillez sélectionner une ligne que vous souhaitez modifier !", QMessageBox.Ok)
            self.show()
...
class LoginForm(QWidget):
    ...
    def check_password(self):
        username = self.ui.lineEdit_Username.text().encode('utf-8')
        password = self.ui.lineEdit_Password.text().encode('utf-8')
        secret = hashlib.sha256(username + password).hexdigest()
        
        if secret == os.environ["AGROTECH_SECRET"]:
